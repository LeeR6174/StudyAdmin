  ...
  return (
    <div className={styles.container}>
      <header className={styles.header} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className={styles.title}>Inbox 一覧 <Database size={24} style={{ color: 'var(--accent-primary)' }} /></h1>
          <p className={styles.subtitle}>{activeItems.length}件の未完了タスク</p>
        </div>
      ...
      <div className={styles.content}>
        {isLoading ? (
          ...
          <div className={styles.emptyState}>
            ...
            <p>タスクはすべて完了しています！<br/>素晴らしい1日を。</p>
          </div>
        ) : (
            ...
            <AnimatePresence mode="popLayout">
              {activeItems.map((item) => (
                ...
                >
                  ...
                  {item.memo && <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.1)', padding: '12px', borderRadius: '12px' }}>{item.memo}</div>}
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
